<script lang="ts">
  ...
  const formatLocaleCurrency = (ticker: string, amount: number) => {
    ...
    return new Intl.NumberFormat(locale, { style: 'currency', currency: ticker }).format(amount);
  }
  ...
  <ModalBody>
    ...
    <div class="checkout-item-box">
      ...
      <div class="checkout-item-name">
        {$transactionStore.destinationAmount.toFixed(8)} {$transactionStore.destinationCurrency.ticker}
      </div>
    </div>
    <div class="line-items">      
      ...
      <div class="line-item muted">
        <div>Network Fee</div>
        <div>{(0).toFixed(8)} {$transactionStore.destinationCurrency.ticker}</div>
      </div> 
      <div class="line-item muted">
        <div>Partner Fee</div>
        <div>{formatLocaleCurrency($transactionStore.sourceCurrency.ticker, 0)}</div>
      </div>
      <div class="line-item muted">
        <div>Exchange Rate</div>
        <div>{formatLocaleCurrency($transactionStore.sourceCurrency.ticker, exchangeRate)}</div>
      </div>       
      <div class="line" />  
      <div class="line-item">
        <div><b>Total</b></div>
        <div>{formatLocaleCurrency($transactionStore.sourceCurrency.ticker, $transactionStore.sourceAmount)}</div>
      </div>
    </div>
  </ModalBody>
  <ModalFooter>
